...
class api_execute extends MX_Controller {
	...
	// Collect extra segments as parameters
		...
		if(array_key_exists($api_method_name, $all_methods)){
			...
			$api_func_with_verb = $api_func ? $api_func . '_' . $request_method_lower : '';

			if ($api_func_with_verb && method_exists($this->$api_method_name, $api_func_with_verb)) {
				// Call method appended with HTTP verb (e.g. delete_post, list_get)
				call_user_func_array([$this->$api_method_name, $api_func_with_verb], $call_params);
			} elseif ($api_func && method_exists($this->$api_method_name, $api_func)) {
				// Fallback: call method without verb suffix
