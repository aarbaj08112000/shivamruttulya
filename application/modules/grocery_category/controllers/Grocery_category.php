...
class Grocery_category extends MY_Controller {
    ...
    public function grocery_category_list()
    {
        /* datatable */
        $column[] = [
            "data" => "id",
        ...
        $this->smarty->loadView('grocery_category_list.tpl', $data,'Yes','Yes');
    }

    public function get_grocery_category_list()
    {
        ...
        $data = $this->Grocery_category_model->get_grocery_category_view_data(
            $condition_arr,
            $post_data["search"]
        );
        ...
        $response = array();
        $response["data"] = $data;
        $total_record = $this->Grocery_category_model->get_grocery_category_view_count([], $post_data["search"]);
    ...
    public function add_grocery_category_action() {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            $insert_id = $this->Grocery_category_model->insert_grocery_category($data);
    ...
    public function get_grocery_category_details() {
        $category_id = $this->input->post('id');
        $category = $this->Grocery_category_model->get_grocery_category_by_id($category_id);
    ...
    public function update_grocery_category_action() {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            $updated = $this->Grocery_category_model->update_grocery_category($category_id, $data);
    ...
    public function delete_grocery_category_action() {
        $response = ['success' => 0, 'msg' => 'An error occurred while deleting the category.'];
        
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            $category_id = $this->input->post('id', TRUE);
            
            $deleted = $this->Grocery_category_model->delete_grocery_category($category_id);
